...
public class FrameBuffer implements AutoCloseable {
    ...
    public FrameBuffer(String deviceName, boolean autoUpdate) {
        ...
        logger.info("FrameBuffer: Open with {} ({})",deviceName, deviceInfo);
        logger.info("             width  = {}",getDeviceWidth(deviceInfo));
        logger.info("             height = {}",getDeviceHeight(deviceInfo));
        logger.info("             bpp    = {}",getDeviceBitsPerPixel(deviceInfo));
    ...
    public boolean updateScreen() {
        ...
        synchronized (deviceName) {
            ...
            synchronized (updateLock) {
                ...
                if (diff >= 1000) {
                    logger.trace("updateScreen: fps = {}",(1000f / diff) * updateCount);
                    updateCount = 0;
                    lastUpdate  = now;
                }
